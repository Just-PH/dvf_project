...
class GAMRegressor(BaseEstimator, RegressorMixin):
    ...
    def __init__(self, term, max_iter=100, lam=0.5):
        ...
        self.lam = lam
        self.gam_ = None  # Will be initialized in fit()
...
def train_combined_model(df_features, df_target, physical_pipeline, physical_features,
                         params_physical, contextual_pipeline, contextual_features,
                         params_contextual, save=False, stacking=False, gam=False):
    """
...
def combined_prediction(model, df_features, physical_pipeline, physical_features,
                        contextual_pipeline, contextual_features):
    """
...
def grid_search_combined_model(df_features, df_target, physical_pipeline, physical_features,
                               params_physical, contextual_pipeline, contextual_features,
                               params_contextual, stacking=False, gam=False, save=False):
    """
...
def predict_wrapper(X, combined_model, len_physical=0):
    """
...
def compute_shap_impact(model, X, physical_pipeline, physical_features,
                        contextual_pipeline, contextual_features, sample_size=None):
    """
